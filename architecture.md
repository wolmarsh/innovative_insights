...
2. **Backend**: A Python web server powered by FastAPI, utilizing PyPDF for extraction, NumPy for vector math (cosine similarity search), and OpenRouter API for language models and embedding generation.
...
graph TD
    ...
    API --> IDX
    PDF -->|Text Chunks| IDX
    IDX -->|Generate Vectors| LLM
    LLM -->|Request Embeddings| OR
    IDX -->|Cache Chunks & Vectors| VEC
    
    QA -->|Query String| API
    API -->|Vector Search| IDX
    IDX -->|Dot Product Search| VEC
    API -->|Prompt Generation| LLM
...
- **Upload Manager**: Manages drag-and-drop actions, reads PDF files as binaries, and uploads them to the server.
...
- **In-Memory Store**: Retains extracted text chunks alongside their metadata (filename, page number) and 1536-dimensional floating-point embedding vectors in a global dictionary keyed by `session_id`.
...
- **Embedding Generation**: Processes text chunks in batches of 16 using OpenRouter's embeddings API (`openai/text-embedding-3-small` by default).
- **RAG Chat completions**: Builds a system prompt instructing the model to ground its response strictly in the retrieved snippets, and calls OpenRouter chat endpoint (`google/gemini-2.5-flash` by default).
...
2. The frontend sends a multipart request to `POST /api/upload` containing the files, the session ID, and the API key.
...
5. The backend requests embeddings for all chunks in parallel batches from OpenRouter.
...
3. The backend calls OpenRouter to generate an embedding vector for the question.
